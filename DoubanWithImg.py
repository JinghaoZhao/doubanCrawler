...
def book_spider(book_tag):
    ...
    while(1):
        ...
        for book_info_all in list_soup.findAll('li',{'class':'subject-item'}):
            # print book_info
            book_info = book_info_all.find('div',{'class':'info'})
            ...
            try:
                ...
                mode = re.compile(r'\d+')
                people_num = int(mode.findall(people_num)[0])
            except:
                people_num = '0'

            # book_image
            # book_img = book_info_all.find('div',{'class':'pic'}).find('img').get('src')
            # imgpath = 'H:\workPlace\PycharmProjects\doubanBook\picture/%s.jpg' % title
            # urllib.urlretrieve(book_img,imgpath)
